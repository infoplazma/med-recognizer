...
# **********************************************************************************************************************
from toolkit.prompt_templates.disease_name_agent_prompts import SYSTEM_PROMPT, HUMAN_PROMPT
# **********************************************************************************************************************
#                                          Установки
# **********************************************************************************************************************
# Инициализация LLM
...
class DiseaseList(TypedDict):
    diseases: Annotated[List[DiseaseName], ..., "List of found and extracted diseases"]


def log_main_extractor(input_dict: dict, config: RunnableConfig):
    return log_chain_step(input_dict, config, "extractor", msg="Извлекаю через основной экстрактор.")


def log_json_branch(input_dict: dict, cfg: RunnableConfig):
    return log_chain_step(input_dict, cfg, "extractor", msg="Пробую через JSONParser")


def log_fallback_branch(_: dict, cfg: RunnableConfig):
    return log_chain_step({}, cfg, "extractor", msg="Пробую через RetryOutputParser")


# **********************************************************************************************************************
#                                          Определение Агента
# **********************************************************************************************************************
def disease_list_agent(page_text: str, config: RunnableConfig = None) -> Dict:
    ...
    structured_extractor = prompt | RunnableLambda(log_main_extractor) | llm.with_structured_output(DiseaseList, method="json_schema")
    ...
    gen_for_retry = RunnableParallel(
        ...
        completion=prompt | RunnableLambda(log_json_branch) | llm | (lambda m: m.content),  # сырой текст модели
    )
    ...
    chain = structured_extractor.with_fallbacks([fallback_extractor]).with_config(config)
...
if __name__ == "__main__":
    ...
    agent_response = disease_list_agent(sample_text, config=CONFIG)
